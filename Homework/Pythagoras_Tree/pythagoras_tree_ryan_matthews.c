#include "FPToolkit.c"
void pythagoras_tree(double x, double y, double size, double angle, int depth);
int main()
{
...
void pythagoras_tree(double x, double y, double size, double angle, int depth) {
    ...
    pythagoras_tree(x3, y3, size * 0.7, angle + M_PI / 4, depth - 1);
    pythagoras_tree(x4, y4, size * 0.7, angle - M_PI / 4, depth - 1);
}